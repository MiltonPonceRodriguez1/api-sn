        ...
        <form method="POST" action="{{ url('publication') }}" enctype="multipart/form-data">
            ...
            <div class="flex flex-1 items-start space-x-4 p-5">
                @if (session('avatar') && session('avatar') != 'default.png')
                    <img src="{{ env('URL_ROOT') }}user/avatar/{{ session('avatar') }}" class="bg-gray-200 border border-white rounded-full w-11 h-11">
                @else
                ...
                <div class="flex-1 pt-2">
                    <textarea name="text" class="uk-textare text-black shadow-none focus:shadow-none text-xl font-medium resize-none" rows="5" placeholder="¿Qué estás pensando?"></textarea>
                </div>

            </div>
            <div class="bsolute bottom-0 p-4 space-x-4 w-full">
                <div class="flex bg-gray-50 border border-purple-100 rounded-2xl p-2 shadow-sm items-center">
                    <div class="lg:block hidden ml-1"> Agregar a tú publicación </div>
                    <div class="flex flex-1 items-center lg:justify-end justify-center space-x-2">

                        <svg id="file-upload-post-image" class="bg-blue-100 h-9 p-1.5 rounded-full text-blue-600 w-9 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {{-- INPUTS --}}
                        <input type="file" id="post-image" name="file0" style="display: none;">
            ...
            <div class="flex items-center w-full justify-between border-t p-3">
                ...
                <div class="flex space-x-2">
                    ...
                    <!-- <a type="submit" class="bg-blue-600 flex h-9 items-center justify-center rounded-md text-white px-5 font-medium">
                        Publicar </a> -->
                </div>
...
<script>
    init_Event('file-upload-post-image', 'post-image');
</script>